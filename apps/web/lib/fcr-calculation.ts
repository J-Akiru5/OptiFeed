...
export function calculateFeedRequirement(input: FcrCalculationInput): FcrCalculationResult {
	...
	const dailyFeedPerFishG = (dailyFeedRationKg * 1000) / POPULATION;
	const totalBiomassG = input.currentBiomassKg * 1000 * POPULATION;
	const suggestedFeedingRatePct =
		totalBiomassG > 0 ? (dailyFeedPerFishG / (totalBiomassG / POPULATION)) * 100 : 0;

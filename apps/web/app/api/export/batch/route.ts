...
export async function GET(request: Request) {
	...
	const types = typesParam
		? typesParam.split(",").filter((t) => EXPORT_TYPES.includes(t as ExportableType))
		: EXPORT_TYPES;
	...
	for (const type of types) {
		...
		if (rows.length > 0) {
			zip.file(`${type}.csv`, toCsvString(rows));
		}
	}
	...
	return new Response(zipBuffer, {
		headers: {
			...
			"Content-Disposition": `attachment; filename="optifeed_export_${new Date().toISOString().slice(0, 10)}.zip"`,
		},
	});
}
